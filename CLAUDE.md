...
npm test          # vitest run — all unit tests, including the web half of the web/Android parity fixture
```
...
An anchor is fixed for a local day, so it is fetched **once per ticker per day**: module-level cache on the web, the `daily_anchors` Room table (`MIGRATION_2_3`) on Android. Only a **404** is cached as a null anchor (the ticker does not exist on Yahoo); a 429, 5xx, dead network or **coroutine cancellation** is never cached and is retried (web after 60 s, Android on the next refresh) — caching those once pinned rows to the fallback until midnight. Pence-quoted (`GBp`) anchors are scaled to GBP like the live quote.
...
### Shared calc + parity fixture (`test-fixtures/`)

Every number both apps show is one pure function written twice — TS in `src/utils/`, Kotlin in `android/core/calc/` — and pinned by a shared JSON case table that **both** test suites read:

| Fixture | Web | Android |
|---|---|---|
| `parity/` — full row + portfolio figures, sort orders, in CZK/USD/EUR | `rowDerivation.ts` (`deriveRow`, `computePortfolioIrr`), `rowSorting.ts` (`portfolioDailyChange`, `sortRowsForDisplay`) — `parity.test.ts` | `RowDerivation.kt`, `PortfolioSummary.kt`, `RowSorting.kt` — `ParityTest.kt` |
| `type-filter/` | `typeFilter.ts` | `TypeFilter.kt` |
| `tax/` — Czech 3-year time test | `taxTest.ts` | `TaxTest.kt` |
| `benchmark/` — cash-flow-matched (PME) line | `benchmark.ts` | `Benchmark.kt` |
| `forecast/` — next-12-months dividends | `dividendForecast.ts` | `DividendForecast.kt` |

`parity/expected.json` is generated by the web (`npx vitest run -u parity`) and asserted by Android to 0.01 — **read the diff by hand before committing a regenerated golden**. `parity/README.md` lists the bugs this found. Rules that must stay identical: a lot is sold iff `sellPrice != null && sellDate` (a sell price of 0 still counts); portfolio today's change sums `dailyChangeDisplay` (already display currency), never the legacy `dailyChange`; portfolio IRR ignores closed rows whose own IRR is null.

`src/utils/chartHistory.ts` holds the Yahoo history/FX helpers (`fetchYahooHistory`, `fetchFxHistory` + session `fxHistCache`, `priceAt`, `makeConvertAt`) shared by `PortfolioPnLChart` and `TaxPanel`; Android's equivalent `makeConvertAt` is in `ChartMath.kt`.

**Features built on these** (web ↔ Android): chart type-filter chips (chart only, never refetches; `localStorage['chart_types_portfolio']` / DataStore `chart_types`), benchmark overlay vs MSCI World (`URTH`) / S&P 500 (`^GSPC`, always fetched full-window; `chart_benchmark_portfolio` / `chart_benchmark`), Czech tax panel + per-lot badges (`TaxPanel`/`TaxBadge` ↔ `TaxCard`/lot card), dividend forecast (`Est. Dividends 12m` card + `DividendCalendar` ↔ `DividendForecastCard`). Tax and forecast are labelled estimates.
...
- `PortfolioRow` — one row per ticker; all aggregated fields plus `positions: Position[]`, `priceIsManual: boolean`, `manualPriceDate?: string`, `irr: number | null`, **`isClosed: boolean`**, **`dailyChange: number`** (absolute daily P&L change = `quote.change × openQty`), `totalQuantity` (every share ever bought) vs **`openQuantity`** (shares still held — the Qty column, its sort and manual-price entry use this one)
...
### Price alerts + widget (Android only)

- **Alerts**: `price_alerts` Room table (**database version 4**, `MIGRATION_3_4`), device-local, never synced. `PriceAlertRepository` schedules the unique periodic `PriceAlertWorker` (15 min, network-constrained) only while an enabled alert exists; `evaluateAlert` (`core:calc/AlertCrossing.kt`) fires once per crossing and re-arms when the price returns. Thresholds are in the row currency. `POST_NOTIFICATIONS` is requested when the first alert is created. UI: `PriceAlertsSection` on the detail screen.
- **Widget**: Glance `PortfolioWidget` (`app/.../widget/`). It never computes: `PortfolioListViewModel` writes a debounced `WidgetSnapshot` (DataStore `widget_snapshot`) of exactly the figures on screen, and `StockTrackerApp` pushes each new snapshot to placed widgets — a background recompute without midnight anchors would disagree with the app.
- **Detail parity**: `PositionFactsSection` (`positionFacts()` in `core:calc`) shows the web table's per-position columns: quantity (bought/sold split), avg buy, first buy, lots, type, return %, brokers, ISIN.
