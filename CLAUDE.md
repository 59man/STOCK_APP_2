...
### Auto-fetched prices for the 3 onemarkets funds + FIO Global Fond

Three UniCredit onemarkets funds (ISINs LU2606422355, LU2606421548, LU2595011649) and FIO Global Fond (FIOG.PR) have no Yahoo/Stooq listing, but each provider's own website widget calls a public, unauthenticated-but-undocumented data endpoint — the app proxies these server-side (CORS blocks a direct browser call) and treats them as a normal live quote:
...
- **Fio Global Fond** — `GET /api/fio-fund/quote`, a dedicated (non-passthrough) route in `server/index.js`: fiofondy.cz's NAV chart is a Nette "signal" endpoint gated by a session cookie, so the route first GETs the fund page to capture `Set-Cookie` (cached ~30 min in `fioCookieCache`), then GETs `?do=getFundChartData` with that cookie + `X-Requested-With: XMLHttpRequest` + matching `Referer`, and passes the raw JSON array through. `fetchFioFundQuote` parses it via `parseFioFundJson`.

`FUND_PROVIDER_TICKERS`/`FUND_PROVIDER_SET` (`src/data/fundProviderTickers.ts`) is the single source of truth for which tickers use which provider — checked in `useQuotes.ts`'s `fetchQuote()` before falling through to the normal Yahoo/Stooq chain. `PortfolioContent.tsx` still excludes these from the dividend fetch (`dividendTickers`) since neither provider exposes dividend history, but no longer excludes them from the quote fetch — `NO_FEED_TICKERS` (`src/data/noFeedTickers.ts`) is now empty, reserved for any future ticker with genuinely no feed anywhere.
